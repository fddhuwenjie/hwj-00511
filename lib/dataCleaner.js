    ...
    return new Promise((resolve, reject) => {
      ...
      reader.read(
        (row, lineNum, rowIndex) => {
          ...
          const uniqueKeysList = this.ruleEngine.getUniqueKeys();
          ...
          if (uniqueKeysList.length === 0) {
            ...
            if (seenFull.has(sig)) {
              isDuplicate = true;
              dupReason = 'full_duplicate';
            } else {
              seenFull.add(sig);
            }
          } else {
            const ukViolations = this.ruleEngine.checkUniqueKey(newRow, lineNum, rowIndex);
            if (ukViolations.length > 0) {
              isDuplicate = true;
              dupReason = ukViolations.map(v => v.message).join('; ');
            }
          }

          const validation = this.ruleEngine.validateRow(newRow, lineNum, rowIndex);
          const nonUkIssues = validation.issues.filter(i => i.type !== 'unique_key');
          if (nonUkIssues.length > 0 || validation.warnings.length > 0) {
            rowIssues.push(...nonUkIssues, ...validation.warnings);
          }

          if (isDuplicate) {
            this.stats.removedDuplicates++;
            this.removedDuplicates.push({ lineNum, row: newRow, reason: dupReason });
            this.changelog.push({ lineNum, action: 'remove_duplicate', reason: dupReason });
            if (options.includeReview) {
              this.reviewRecords.push({
                lineNum,
                action: 'duplicate',
                problems: ['重复行: ' + dupReason],
                suggestions: ['删除'],
