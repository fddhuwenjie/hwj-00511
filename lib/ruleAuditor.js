...
class RuleAuditor {
  ...
  _analyzeCrossFieldRules(rules) {
    ...
    if (crossRules.length === 0) {
      const datePairs = this._discoverDatePairs(rules);
  ...
  _isLikelyDateField(field, rules) {
    const rule = rules?.fields?.[field];
    if (rule?.type === 'date' || rule?.type === 'datetime') return true;
    if (rule?.type === 'integer' || rule?.type === 'number' || rule?.type === 'boolean') return false;

    const lower = field.toLowerCase();
    const dateKeywords = [
      'date', 'time', 'datetime', 'timestamp', 'created', 'updated',
      'register', 'delivery', 'publish', 'expired', 'expire',
      '出生', '日期', '时间', '创建', '更新', '注册', '下单', '发货',
      '开始', '结束', '完成', '到达', '到期'
    ];
    const hasDateKeyword = dateKeywords.some(kw => lower.includes(kw.toLowerCase()));

    const values = this.columnData[field];
    const nonEmpty = values.filter(v => v !== '' && v !== null && v !== undefined);
    if (nonEmpty.length < 5) return false;

    let explicitFormatCount = 0;
    ...
    for (const v of nonEmpty) {
      ...
      for (const p of DATE_PATTERNS) {
        if (p.format === 'Unix ms timestamp' || p.format === 'Unix timestamp' || p.format === 'other') continue;
        if (p.regex.test(s)) {
          isExplicit = true;
          break;
        }
    ...
    const explicitRatio = explicitFormatCount / nonEmpty.length;
    const pureNumberRatio = pureNumberCount / nonEmpty.length;
    const tsRatio = tsCount / nonEmpty.length;

    if (explicitRatio > 0.5) return true;

    if (tsRatio > 0.8) return true;

    if (pureNumberRatio > 0.8 && tsRatio < 0.1 && !hasDateKeyword) return false;

    if (hasDateKeyword && tsRatio > 0.3) return true;

    if (hasDateKeyword && explicitRatio > 0.3) return true;

    return false;
  }

  _discoverDatePairs(rules) {
    const dateFields = [];
    for (const field of this.header) {
      if (this._isLikelyDateField(field, rules)) {
